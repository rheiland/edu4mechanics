...
def write_config_file_cb(b):
    path_to_share = os.path.join('~', '.local','share','edu4mechanics')
...
def get_config_files():
    ...
    if nanoHUB_flag:
        full_path = os.path.expanduser("~/data/results/.submit_cache/edu4mechanics")  # does Windows like this?
    else:
        # local cache
        try:
            cachedir = os.environ['CACHEDIR']
            full_path = os.path.join(cachedir, "edu4mechanics")
        except:
...
def run_done_func(s, rdir):
    ...
    if nanoHUB_flag:
        ...
        os.system("submit  mail2self -s 'nanoHUB edu4mechanics' -t 'Your Run completed.'&")
...
#         # something on NFS causing issues...
    ...
    if nanoHUB_flag:
        if remote_cb.value:
            s.run(run_name, "-v ncn-hub_M@brown -n 8 -w 1440 edu4mechanics-r7 config.xml")   # "-r7" suffix??
        else:
...
titles = ['About', 'Config Basics', 'User Params', 'Out: Cells', 'Code' ]
...
tabs = widgets.Tab(children=[about_tab.tab, config_tab.tab, user_tab.tab, cells_tab.tab, code_tab.tab],
                   _titles={i: t for i, t in enumerate(titles)},
                   layout=tab_layout)

homedir = os.getcwd()

tool_title = widgets.Label(r'\(\textbf{Mechanics Edu App}\)')
if nanoHUB_flag or hublib_flag:
